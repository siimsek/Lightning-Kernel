...
SECONDS=0
ZIPNAME="Surge-ginkgo-$(date '+%Y%m%d-%H%M').zip"
...
make -j$(nproc --all) O=out ARCH=arm64 CC=clang AR=llvm-ar AS=llvm-as NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi- Image.gz-dtb dtbo.img
fi
